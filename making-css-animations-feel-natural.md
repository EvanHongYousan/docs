...
不是很令人兴奋。 虽然，公平地说，这符合所有的标准。 不过，我们绝对可以做得比这更好。

首先要看的是定时功能。 在上面的例子中，我们使用线性定时功能，这意味着盒子以相同的速度不断移动。 在某些情况下，这是可取的; 然而，在现实世界中，运动通常不会像那样工作。

一个简单的解决方法是简单地改变定时功能以减轻负担。 这使得每个动画的开始和结束比中间部分稍慢，这为一些动画添加了更自然的外观。 以下是启用了缓动功能的框：
...
通过在框的比例上添加一些细微的扭曲，我们可以在动画中添加很多内容：
